...
router.get('/system-config/page-agent', systemConfigController.getPageAgentConfig);
router.post('/system-config/page-agent', verifyToken, systemConfigController.updatePageAgentConfig);

//-----------------------------------AI AGENT PROXY-----------------------------------
router.post('/ai-agent/chat/completions', async (req, res) => {
  try {
    const apiKey = require('../../config').VITE_PAGE_AGENT_API_KEY || '';
    if (!apiKey) {
      return res.status(400).json({ error: 'OpenRouter API Key (VITE_PAGE_AGENT_API_KEY) is not configured in the server environment.' });
    }
    ...
    console.log(`✦ Proxying request to OpenRouter API using model: ${body.model}`);

    // If forwarding to OpenRouter, apply fallback models for free tier to prevent rate limit blocks
